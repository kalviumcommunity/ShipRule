...
def rerank_score(
    ...
    # Combine vector score (weight 0.6), term coverage (weight 0.3), and phrase bonus (weight 0.1)
    vec_component = max(0.0, float(vector_score))
    combined_score = (0.6 * vec_component) + (0.3 * term_coverage) + (0.1 * phrase_bonus)
